...
def user_input(): 
    mf_struct.s = input("Enter the projected values")
    mf_struct.n = int(input("Enter the number of grouping variables"))
    mf_struct.v = input("Enter the list of group by attributes").split(",")
    mf_struct.F = input("Enter the list of aggregate functions").split(",")
    mf_struct.sigma = input("Enter the list of grouping variable predicates").split(",")
    mf_struct.G = input("Enter the predicate for the having clause")
    print(mf_struct.s)
    print(mf_struct.n)
    print(mf_struct.v)
    print(mf_struct.F)
    print(mf_struct.sigma)
    print(mf_struct.G)
    return mf_struct
    
def file_input():
    file_name = input("Enter the filename to read the query from: ").strip()
    return file_name 
...
def process_info(select, group_by, such_that, having, mf_struct, schemaData):
    ...
    ## changed thisssssss for mf_struct input
    # if mf_input: 
    #     select = mf_input
    # else: 
    select = select.split(",")
...
# if file_name: 
choice = input("Enter in m for manual entry and f for file input").strip().lower()
if choice == "m":
    mf_struct = user_input()
    #replace these with mf_struct.s, mf_struct.G, .. ??
...
def main():
    # choice = input("Do you want to enter the phi operators manually (type 'm') or read from a file (type 'f')? ").strip().lower()
    # if choice == "m":
    #     mf_inputs = user_input()
